...
export default function OnlineEditor() {
  ...
  const openProject = async (projectId) => {
    await fetch(`${API}/api/active/${projectId}`, { method: 'POST' });
  ...
  const runPromo = async () => {
    if (!current) return;
    if (!promoTarget) { alert('Pick Frontend or Backend'); return; }
    setPromoBusy(true);
    setPromoTweet('');
    try {
  ...
  useEffect(() => {
    ...
    const url = URL.createObjectURL(new Blob([frontendText], { type: 'text/html' }));
    setBlobUrl(url);
  }, [frontendText, view]);
    ...
    <div className="editor-main-container">
      <div className="editor-left-panel">
        <div className="editor-preview-info">
          ...
          <div>
            Health: Mongo = {health.mongo || 'checking'}
          </div>
        </div>
        <iframe
          ref={iframeRef}
      ...
      <div className="editor-right-panel">
        ...
        {mpInfo.detected && (
          <div className="editor-multipage-info">
            <div><strong>Multiple pages detected</strong></div>
            <div className="editor-pages-list">
              Pages: {mpInfo.pages.join(', ')}{mpInfo.defaultPage ? ` • Default: ${mpInfo.defaultPage}` : ''}
            </div>
            {mpInfo.hints.length > 0 && (
              <div className="editor-hints">
                {mpInfo.hints.map((h, i) => <div key={i}>⚠️ {h}</div>)}
              </div>
            )}
        ...
        <details open className="editor-section">
          <summary><strong>Frontend (editable)</strong></summary>
          <textarea
            value={frontendText}
        ...
        <details className="editor-section">
          <summary><strong>Backend (editable)</strong></summary>
          <textarea
            value={backendText}
        ...
        <details className="editor-section">
          <summary><strong>Data (read-only)</strong></summary>
          {DataTab}
          <div className="editor-note">
        ...
        <div className="editor-promo-section">
          ...
          <div className="editor-promo-controls">
            ...
            {promoTweet && (
              <span className="editor-tweet">Tweet: {promoTweet}</span>
            )}
          </div>
  ...
  return (
    <div className="online-editor">
      <div className="editor-top-bar">
        <div><strong>FindVirtual.me — Online Editor</strong></div>
